...
##############################################################################
#basic understanding of the NPV
...
#check the probability that NPV is negative
...
df_VaR.boot <- data.frame(VaR.boot = VaR.boot)
ggplot(df_VaR.boot) +
  geom_histogram(aes(x = VaR.boot), bins = 10) +
  geom_vline(aes(xintercept = VaR.boot.U, color = "Upper Bound")) +
  geom_vline(aes(xintercept = VaR.boot.L, color = "Lower Bound")) +
  xlab("VaR - Million USD") +
  ylab("Frequency") +
  labs(title = "Possible Value at Risk with 95% Confidence Interval")+
  theme_classic() +
...
ggplot(df_ES.boot) +
  geom_histogram(aes(x = ES.boot), bins = 10) +
  geom_vline(aes(xintercept = ES.boot.U, color = "Upper Bound")) +
  geom_vline(aes(xintercept = ES.boot.L, color = "Lower Bound")) +
  xlab("ES - Million USD") +
  ylab("Frequency") +
  labs(title = "Possible Expected Shortfall with 95% Confidence Interval")+
  theme_classic() +
  theme(legend.title=element_blank())
...
ggplot(well) +
  ...
  geom_vline(aes(xintercept = VaR, color = "5% VaR")) +
  geom_vline(aes(xintercept = VaR.boot.U, color = "Upper Bound VaR"), linetype = "dotted")+
  geom_vline(aes(xintercept = VaR.boot.L, color = "Lower Bound VaR"),linetype = "dotted")+
  theme_classic() +
  theme(legend.title=element_blank())

#drilling cost
drilling <- data.frame(drilling = drilling_all/1000000)
VaR_drilling <- quantile(drilling_all/1000000, 0.95)
...
ggplot(drilling) +
  ...
  geom_vline(aes(xintercept = median(drilling_all/1000000), color = "Median")) +
  geom_vline(aes(xintercept = VaR_drilling, color = "95% VaR"))+
  theme_classic() +
  theme(legend.title=element_blank())
summary(drilling)

#proportion of wet well
VaR_pp <- quantile(pp, 0.05)
pp_1 <- data.frame(pp)
ggplot(pp_1) +
  geom_histogram(aes(x = pp), bins = 20) +
  xlab("Proportion of Wet Well") +
  ylab("Frequency") +
  labs(title = "Possible Proportion of Wet Well Distribution")+
  geom_vline(aes(xintercept = median(pp), color = "Median")) +
  geom_vline(aes(xintercept = VaR_pp, color = "5% VaR"))+
  theme_classic() +
  theme(legend.title=element_blank())
summary(pp)
#NPV
npv_1 <- data.frame(npv = NPV_all/1000000)
var_npv <- quantile(NPV_all/1000000, 0.05)
ggplot(npv_1) +
  ...
  geom_vline(aes(xintercept = var_npv, color = "5% VaR"))+
  geom_vline(aes(xintercept = median(npv), color = "Median"))+
  theme_classic() +
  theme(legend.title=element_blank())
summary(npv_1)
#check the probability NPV goes to negative
length(NPV_all[NPV_all<0])/length(NPV_all)

#dry cost
dry_cost_1 <- data.frame(dry_cost = dry_cost_all/1000000)
var_drycost_1 <- quantile(dry_cost_all/1000000,0.95)
ggplot(dry_cost_1) +
  ...
  geom_vline(aes(xintercept = var_drycost_1, color = "95% VaR"))+
  theme_classic() +
  theme(legend.title=element_blank())
summary(dry_cost_1)
